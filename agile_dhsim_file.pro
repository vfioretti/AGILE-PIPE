...
; ---------> ASCII files
; - G4_GAMS_XPLANE_AGILE<version>_<phys>List_<sim_type>_<strip>_<point>_<n_in>ph_<energy>MeV_<theta>_<phi>.<file>.dat
; - G4_GAMS_YPLANE_AGILE<version>_<phys>List_<sim_type>_<strip>_<point>_<n_in>ph_<energy>MeV_<theta>_<phi>.<file>.dat
; - G4_GAMS_AC_AGILE<version>_<phys>List_<sim_type>_<strip>_<point>_<n_in>ph_<energy>MeV_<theta>_<phi>.<file>.dat
; - G4_GAMS_CAL_AGILE<version>_<phys>List_<sim_type>_<strip>_<point>_<n_in>ph_<energy>MeV_<theta>_<phi>.<file>.dat
...
; ---------> FITS files
; - G4.RAW.AGILE<version>.<phys>List.<sim_type>.<strip>.<point>.<n_in>ph.<energy>MeV.<theta>.<phi>.<file>.fits
; - L0.AGILE<version>.<phys>List.<sim_type>.<strip>.<point>.<n_in>ph.<energy>MeV.<theta>.<phi>.<file>.fits
...
filepath = run_path + '/AGILE'+agile_version+sdir+'/theta'+strtrim(string(theta_type),1)+'/'+stripDir+py_dir+'/'+ene_type+'MeV.'+sim_name+'.'+strtrim(string(theta_type),1)+'theta.'+strtrim(string(N_in),1)+part_type
print, 'BoGEMMS simulation path: ', filepath

outdir = './AGILE'+agile_version+sdir+'/theta'+strtrim(string(theta_type),1)+'/'+stripDir+py_dir+'/'+sim_name+'/'+ene_type+'MeV/'+strtrim(string(N_in),1)+part_type
print, 'BoGEMMS outdir path: ', outdir

CheckOutDir = DIR_EXIST( outdir)
if (CheckOutDir EQ 0) then spawn,'mkdir -p ./AGILE'+agile_version+sdir+'/theta'+strtrim(string(theta_type),1)+'/'+stripDir+py_dir+'/'+sim_name+'/'+ene_type+'MeV/'+strtrim(string(N_in),1)+part_type
...
for ifile=0, n_fits-1 do begin
    ...
    ; Reading the tracker (events with E > 0)                
     if ((struct(k).VOLUME_ID GE tracker_vol_start) or (struct(k).MOTHER_ID GE tracker_vol_start)) then begin
      if (struct(k).E_DEP GT 0.d) then begin        
         ...
         if (struct(k).EVT_ID NE event_id(n_elements(event_id)-1)) then begin
          event_id_conv = [event_id_conv, struct(k).EVT_ID]
          vol_id_conv = [vol_id_conv, struct(k).VOLUME_ID]
          if (isStrip EQ 1) then moth_id_conv = [moth_id_conv, struct(k).MOTHER_ID] else moth_id_conv = [moth_id_conv, 0]
          energy_dep_conv = [energy_dep_conv, struct(k).E_DEP]
          ent_x_conv = [ent_x_conv, struct(k).X_ENT]
          ent_y_conv = [ent_y_conv, struct(k).Y_ENT]
          ent_z_conv = [ent_z_conv, struct(k).Z_ENT]
          exit_x_conv = [exit_x_conv, struct(k).X_EXIT]
          exit_y_conv = [exit_y_conv, struct(k).Y_EXIT]
          exit_z_conv = [exit_z_conv, struct(k).Z_EXIT]
         endif
    ...
    event_id_conv = event_id_conv[1:*]
    vol_id_conv = vol_id_conv[1:*]
    moth_id_conv = moth_id_conv[1:*]
    energy_dep_conv = energy_dep_conv[1:*]
    ent_x_conv = (ent_x_conv[1:*])/10.
    ent_y_conv = (ent_y_conv[1:*])/10.
    ent_z_conv = (ent_z_conv[1:*])/10.
    exit_x_conv = (exit_x_conv[1:*])/10.
    exit_y_conv = (exit_y_conv[1:*])/10.
    exit_z_conv = (exit_z_conv[1:*])/10.
    ...
    ; for conversion 
    Strip_id_conv = dblarr(n_elements(vol_id_conv))
    Si_id_conv = intarr(n_elements(vol_id_conv))
    tray_id_conv = intarr(n_elements(vol_id_conv))

    print, n_elements(vol_id_conv)
    print, n_elements(Strip_id_conv)

    ; Si_id_conv: 0 = x, 1 = y
    for j=0l, n_elements(vol_id_conv)-1 do begin

      if (isStrip EQ 0) then begin   ;--------> STRIP = 0
        div_id = (vol_id_conv(j))/tracker_vol_start
        div_type = vol_id_conv(j) mod tracker_vol_start
        if (div_type EQ 0) then begin
          Si_id_conv(j) = 0
          tray_id_conv(j) = div_id
        endif else begin
          Si_id_conv(j) = 1
          tray_id_conv(j) = (vol_id_conv(j)-tracker_x_y_diff)/tracker_vol_start
        endelse
        Strip_id_conv(j) = 0
      endif else begin
        if (Repli EQ 1) then begin    ;--------> STRIP = 1, REPLI = 1
          div_id = (moth_id(j))/tracker_vol_start
          div_type = moth_id(j) mod tracker_vol_start
          Strip_id_conv(j) = vol_id_conv(j) - tracker_vol_start
          if (div_type EQ 0) then begin
            Si_id_conv(j) = 0
            tray_id_conv(j) = div_id
          endif else begin
          ...
          where_greater_X = where(Si_X_arr GT vol_id_conv(j))
          where_greater_Y = where(Si_Y_arr GT vol_id_conv(j))

          if (where_greater_X(0) NE -1) then begin
            tray_id_conv_temp = where_greater_X(0)
            if (where_greater_X(0) EQ where_greater_Y(0)) then Si_id_conv_temp = tracker_x_y_diff else Si_id_conv_temp = 0
          endif else begin
            Si_id_conv_temp = 0
            tray_id_conv_temp = n_elements(Si_X_arr)
          endelse

          tray_id_conv(j) = tray_id_conv_temp
          if (Si_id_conv_temp EQ 0) then Si_id_conv(j) = 0 else Si_id_conv(j) = 1

          Strip_id_conv_temp = vol_id_conv(j) - tray_id_conv_temp*tracker_vol_start - Si_id_conv_temp
          Strip_id_conv(j) = Strip_id_conv_temp
          if (Strip_id_conv(j) EQ 3072) then begin
          endif
    ...
    for j=0l, n_elements(tray_id)-1 do begin
 	...
 	if (out_type EQ 0) then begin   
	    ...
	    MWRFITS, rawData, outdir+'/G4.RAW.AGILE'+agile_version+'.'+py_name+'.'+sim_name+'.'+stripname+'.'+sname+'.'+strmid(strtrim(string(N_in),1),0,10)+part_type+'.'+ene_type+'MeV.'+strmid(strtrim(string(theta_type),1),0,10)+'.'+strmid(strtrim(string(phi_type),1),0,10)+'.'+strtrim(string(ifile),1)+'.fits', hdr_rawData, /create
    
     print, '%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%'
	    print, '           Saving the raw FIRST hits (fits)      '
	    print, '%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%'

	    ; Conversion from tray ID (starting from bottom) to plane ID (starting from the top)

	    plane_id_conv = intarr(n_elements(tray_id_conv))

	    for j=0l, n_elements(tray_id_conv)-1 do begin
	      if (Si_id_conv(j) EQ 0) then begin
	        temp_plane_x = tray_id_conv(j) - N_plane
	        if (temp_plane_x GT 0) then plane_id_conv(j) = temp_plane_x else plane_id_conv(j) = (-1.*temp_plane_x) + 2
	      endif
	      if (Si_id(j) EQ 1) then begin
	        temp_plane_y = tray_id_conv(j) - (N_plane-1)
	        if (temp_plane_y GT 0) then plane_id_conv(j) = temp_plane_y else plane_id_conv(j) = (-1.*temp_plane_y) + 2
	      endif
	    endfor

	    CREATE_STRUCT, rawFirstData, 'rawFirstData', ['EVT_ID', 'TRK_FLAG', 'TRAY_ID', 'PLANE_ID', 'STRIP_ID', 'E_DEP', 'X_ENT', 'Y_ENT', 'Z_ENT', 'X_EXIT', 'Y_EXIT', 'Z_EXIT'], $
	      'I,I,I,I,I,F20.5,F20.5,F20.5,F20.5,F20.5,F20.5,F20.5', DIMEN = n_elements(event_id_conv)
	    rawFirstData.EVT_ID = event_id_conv
	    rawFirstData.TRK_FLAG = Si_id_conv
	    rawFirstData.TRAY_ID = tray_id_conv
	    rawFirstData.PLANE_ID = plane_id_conv
	    rawFirstData.STRIP_ID = Strip_id_conv
	    rawFirstData.E_DEP = energy_dep_conv
	    rawFirstData.X_ENT = ent_x_conv
	    rawFirstData.Y_ENT = ent_y_conv
	    rawFirstData.Z_ENT = ent_z_conv
	    rawFirstData.X_EXIT = exit_x_conv
	    rawFirstData.Y_EXIT = exit_y_conv
	    rawFirstData.Z_EXIT = exit_z_conv


	    hdr_rawFirstData = ['COMMENT  AGILE '+agile_version+' Geant4 simulation', $
	      'N_in     = '+strtrim(string(N_in),1), $
	    ...
	    MWRFITS, rawFirstData, outdir+'/G4.FIRST.AGILE'+agile_version+'.'+py_name+'.'+sim_name+'.'+stripname+'.'+sname+'.'+strmid(strtrim(string(N_in),1),0,10)+part_type+'.'+ene_type+'MeV.'+strmid(strtrim(string(theta_type),1),0,10)+'.'+strmid(strtrim(string(phi_type),1),0,10)+'.'+strtrim(string(ifile),1)+'.fits', hdr_rawFirstData, /create
    	...
    	openw,lun,outdir+'/G4.RAW.GENERAL.AGILE'+agile_version+'.'+py_name+'.'+sim_name+'.'+stripname+'.'+sname+'.'+strmid(strtrim(string(N_in),1),0,10)+part_type+'.'+ene_type+'MeV.'+strmid(strtrim(string(theta_type),1),0,10)+'.'+strmid(strtrim(string(phi_type),1),0,10)+'.'+strtrim(string(ifile),1)+'.dat',/get_lun
    	; ASCII Columns:
    	; - c1 = event ID
    	; - c2 = Silicon layer ID
    	; - c3 = x/y pos [cm]
    ...
    if (out_type EQ 0) then begin
      ...
      MWRFITS, L0TRACKERGLOBAL, outdir+'/L0.AGILE'+agile_version+'.'+py_name+'.'+sim_name+'.'+stripname+'.'+sname+'.'+STRMID(STRTRIM(STRING(N_IN),1),0,10)+part_type+'.'+ene_type+'MeV.'+STRMID(STRTRIM(STRING(THETA_TYPE),1),0,10)+'.'+STRMID(STRTRIM(STRING(PHI_TYPE),1),0,10)+'.'+strtrim(string(ifile),1)+'.fits', HDR_L0GLOBAL, /CREATE
    endif   
    ...
    if (out_type EQ 0) then begin
      ...
      MWRFITS, L05TRACKERGLOBAL, outdir+'/L0.5.DIGI.AGILE'+agile_version+'.'+py_name+'.'+sim_name+'.'+stripname+'.'+sname+'.'+STRMID(STRTRIM(STRING(N_IN),1),0,10)+part_type+'.'+ene_type+'MeV.'+STRMID(STRTRIM(STRING(THETA_TYPE),1),0,10)+'.'+STRMID(STRTRIM(STRING(PHI_TYPE),1),0,10)+'.'+strtrim(string(ifile),1)+'.fits', HDR_L05GLOBAL, /CREATE
      ...
      openw,lun,outdir+'/G4.DIGI.KALMAN.AGILE'+agile_version+'.'+py_name+'.'+sim_name+'.'+stripname+'.'+sname+'.'+strmid(strtrim(string(N_in),1),0,10)+part_type+'.'+ene_type+'MeV.'+strmid(strtrim(string(theta_type),1),0,10)+'.'+strmid(strtrim(string(phi_type),1),0,10)+'.'+strtrim(string(ifile),1)+'.dat',/get_lun
      ; ASCII Columns:
      ; - c1 = event ID
      ; - c2 = Silicon layer ID
      ; - c3 = x/y pos [cm]
      ...
      openw,lun,outdir+'/G4.DIGI.GENERAL.AGILE'+agile_version+'.'+py_name+'.'+sim_name+'.'+stripname+'.'+sname+'.'+strmid(strtrim(string(N_in),1),0,10)+part_type+'.'+ene_type+'MeV.'+strmid(strtrim(string(theta_type),1),0,10)+'.'+strmid(strtrim(string(phi_type),1),0,10)+'.'+strtrim(string(ifile),1)+'.dat',/get_lun
      ; ASCII Columns:
      ; - c1 = event ID
      ; - c2 = Silicon layer ID
      ; - c3 = x/y pos [cm]
  ...
  openw,lun,outdir+'/G4.AA.STRIP.AGILE'+agile_version+'.'+py_name+'.'+sim_name+'.'+stripname+'.'+sname+'.'+strmid(strtrim(string(N_in),1),0,10)+part_type+'.'+ene_type+'MeV.'+strmid(strtrim(string(theta_type),1),0,10)+'.'+strmid(strtrim(string(phi_type),1),0,10)+'.'+strtrim(string(ifile),1)+'.dat',/get_lun
  ; ASCII Columns:
  ; - c1 = event ID
  ; - c2 = theta input
  ; - c3 = phi input
  ...
  while (1) do begin
	...
	if (where_x(0) NE -1) then begin
	  for r=0l, n_elements(where_x)-1 do begin
		printf, lun, Glob_event_id_acap(j), theta_type, phi_type, ene_type, Glob_plane_id_acap_temp(where_x(r)), Glob_zpos_acap_temp(where_x(r)), 0, Glob_Strip_id_acap_temp(where_x(r)), Glob_pos_acap_temp(where_x(r)), Glob_energy_dep_acap_temp(where_x(r)), 0, format='(I7,I7,I7,I7,I7,F20.7,I7,I7,F20.7,F20.7,I7)'
	  endfor
	endif
	...
	if (where_y(0) NE -1) then begin
	  for r=0l, n_elements(where_y)-1 do begin
		printf, lun, Glob_event_id_acap(j), theta_type, phi_type, ene_type, Glob_plane_id_acap_temp(where_y(r)), Glob_zpos_acap_temp(where_y(r)), 1, Glob_Strip_id_acap_temp(where_y(r)), Glob_pos_acap_temp(where_y(r)), Glob_energy_dep_acap_temp(where_y(r)), 0, format='(I7,I7,I7,I7,I7,F20.7,I7,I7,F20.7,F20.7,I7)'
	  endfor
	endif
	; ------------------------------------
  ...
  print, '%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%'
  print, '                      Tracker   '
  print, '       		cluster baricenter '
  print, '%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%'


  Glob_event_id_cluster = -1l
  Glob_Si_id_acap_cluster = -1l
  Glob_tray_id_acap_cluster = -1l
  Glob_plane_id_acap_cluster = -1l
  Glob_pos_acap_cluster = -1.
  Glob_zpos_acap_cluster = -1.
  Glob_energy_dep_acap_cluster = -1.
  Glob_Strip_number_acap_cluster = -1l
  Glob_pair_flag_acap_cluster = -1l
  ...
  while (1) do begin
	...
	where_event_eq = where(Glob_event_id_acap EQ Glob_event_id_acap(j))
	;print, 'event ID: ', Glob_event_id_acap(j)
	Glob_Si_id_acap_temp = Glob_Si_id_acap(where_event_eq)
	Glob_Strip_id_acap_temp = Glob_Strip_id_acap(where_event_eq)
	Glob_tray_id_acap_temp  = Glob_tray_id_acap(where_event_eq)
	Glob_plane_id_acap_temp  = Glob_plane_id_acap(where_event_eq)
	...
	; sorting the planes
	sort_ascending_plane = sort(Glob_plane_id_acap_temp)
	Glob_Si_id_acap_temp_sortplane = Glob_Si_id_acap_temp[sort_ascending_plane]
	Glob_Strip_id_acap_temp_sortplane = Glob_Strip_id_acap_temp[sort_ascending_plane]
	Glob_tray_id_acap_temp_sortplane = Glob_tray_id_acap_temp[sort_ascending_plane]
	Glob_plane_id_acap_temp_sortplane = Glob_plane_id_acap_temp[sort_ascending_plane]
	Glob_energy_dep_acap_temp_sortplane = Glob_energy_dep_acap_temp[sort_ascending_plane]
	Glob_pos_acap_temp_sortplane = Glob_pos_acap_temp[sort_ascending_plane]
	Glob_zpos_acap_temp_sortplane = Glob_zpos_acap_temp[sort_ascending_plane]
	...
	pl=0l
  	while (1) do begin
		...
		where_plane_eq = where(Glob_plane_id_acap_temp_sortplane EQ Glob_plane_id_acap_temp_sortplane(pl))
		;print, 'plane: ', Glob_plane_id_acap_temp_sortplane(pl)
		Glob_Si_id_acap_sameplane = Glob_Si_id_acap_temp_sortplane(where_plane_eq)
		Glob_Strip_id_acap_sameplane = Glob_Strip_id_acap_temp_sortplane(where_plane_eq)
		Glob_tray_id_acap_sameplane  = Glob_tray_id_acap_temp_sortplane(where_plane_eq)
		Glob_energy_dep_acap_sameplane = Glob_energy_dep_acap_temp_sortplane(where_plane_eq)    
		Glob_pos_acap_sameplane = Glob_pos_acap_temp_sortplane(where_plane_eq)
		Glob_zpos_acap_sameplane = Glob_zpos_acap_temp_sortplane(where_plane_eq)
		
		; ------------------------------------
		; X VIEW
		where_x = where(Glob_Si_id_acap_sameplane EQ 0)
		if (where_x(0) NE -1) then begin
			Glob_Strip_id_acap_x = Glob_Strip_id_acap_sameplane[where_x]
			Glob_tray_id_acap_x = Glob_tray_id_acap_sameplane[where_x]
			Glob_energy_dep_acap_x = Glob_energy_dep_acap_sameplane[where_x]
			Glob_pos_acap_x = Glob_pos_acap_sameplane[where_x]
			Glob_zpos_acap_x = Glob_zpos_acap_sameplane[where_x]
			
			;sorting the strips
			sort_ascending_strip = sort(Glob_Strip_id_acap_x)
			Glob_Strip_id_acap_x_sort = Glob_Strip_id_acap_x[sort_ascending_strip]
			Glob_tray_id_acap_x_sort = Glob_tray_id_acap_x[sort_ascending_strip]
			Glob_energy_dep_acap_x_sort = Glob_energy_dep_acap_x[sort_ascending_strip]
			Glob_pos_acap_x_sort = Glob_pos_acap_x[sort_ascending_strip]
			Glob_zpos_acap_x_sort = Glob_zpos_acap_x[sort_ascending_strip]
			
			e_cluster_temp = Glob_energy_dep_acap_x_sort(0)
            wx_cluster_temp = Glob_pos_acap_x_sort(0)*Glob_energy_dep_acap_x_sort(0)
            nstrip_temp = 1

            if (n_elements(Glob_Strip_id_acap_x_sort) EQ 1) then begin
            
            	Glob_event_id_cluster = [Glob_event_id_cluster, Glob_event_id_acap(j)]
            	Glob_Si_id_acap_cluster = [Glob_Si_id_acap_cluster, 0]
            	Glob_tray_id_acap_cluster = [Glob_tray_id_acap_cluster, Glob_tray_id_acap_x_sort(0)]
            	Glob_plane_id_acap_cluster = [Glob_plane_id_acap_cluster, Glob_plane_id_acap_temp_sortplane(pl)]
  				Glob_pos_acap_cluster =[Glob_pos_acap_cluster, total(wx_cluster_temp)/total(e_cluster_temp)]
  				Glob_zpos_acap_cluster = [Glob_zpos_acap_cluster, Glob_zpos_acap_x_sort(0)]
  				Glob_energy_dep_acap_cluster = [Glob_energy_dep_acap_cluster, total(e_cluster_temp)]
  				Glob_Strip_number_acap_cluster = [Glob_Strip_number_acap_cluster, nstrip_temp]
  				Glob_pair_flag_acap_cluster = [Glob_pair_flag_acap_cluster, 0]
    
            endif else begin

              for jc=0l, n_elements(Glob_Strip_id_acap_x_sort) -2 do begin
                if (Glob_Strip_id_acap_x_sort(jc+1) EQ (Glob_Strip_id_acap_x_sort(jc)+2)) then begin
                  e_cluster_temp = [e_cluster_temp, Glob_energy_dep_acap_x_sort(jc+1)]
                  wx_cluster_temp = [wx_cluster_temp, Glob_pos_acap_x_sort(jc+1)*Glob_energy_dep_acap_x_sort(jc+1)]
                  nstrip_temp = nstrip_temp+1

                  if (jc EQ (n_elements(Glob_Strip_id_acap_x_sort)-2)) then begin
                    Glob_event_id_cluster = [Glob_event_id_cluster, Glob_event_id_acap(j)]
                    Glob_Si_id_acap_cluster = [Glob_Si_id_acap_cluster, 0]
                    Glob_tray_id_acap_cluster = [Glob_tray_id_acap_cluster, Glob_tray_id_acap_x_sort(jc)]
                    Glob_plane_id_acap_cluster = [Glob_plane_id_acap_cluster, Glob_plane_id_acap_temp_sortplane(pl)]
                    Glob_zpos_acap_cluster = [Glob_zpos_acap_cluster, Glob_zpos_acap_x_sort(jc)]
                    Glob_energy_dep_acap_cluster = [Glob_energy_dep_acap_cluster, total(e_cluster_temp)]
                    Glob_pos_acap_cluster =[Glob_pos_acap_cluster, total(wx_cluster_temp)/total(e_cluster_temp)]
                    Glob_Strip_number_acap_cluster = [Glob_Strip_number_acap_cluster, nstrip_temp]
                    Glob_pair_flag_acap_cluster = [Glob_pair_flag_acap_cluster, 0]
                  ...
                  e_cluster_temp = Glob_energy_dep_acap_x_sort(jc+1)
                  wx_cluster_temp = Glob_pos_acap_x_sort(jc+1)*Glob_energy_dep_acap_x_sort(jc+1)
                  nstrip_temp = 1

                  if (jc EQ (n_elements(Glob_Strip_id_acap_x_sort)-2)) then begin
                    Glob_event_id_cluster = [Glob_event_id_cluster, Glob_event_id_acap(j)]
                    Glob_Si_id_acap_cluster = [Glob_Si_id_acap_cluster, 0]
                    Glob_tray_id_acap_cluster = [Glob_tray_id_acap_cluster, Glob_tray_id_acap_x_sort(jc+1)]
                    Glob_plane_id_acap_cluster = [Glob_plane_id_acap_cluster, Glob_plane_id_acap_temp_sortplane(pl)]
                    Glob_zpos_acap_cluster = [Glob_zpos_acap_cluster, Glob_zpos_acap_x_sort(jc+1)]
                    Glob_energy_dep_acap_cluster = [Glob_energy_dep_acap_cluster, Glob_energy_dep_acap_x_sort(jc+1)]
				    Glob_pos_acap_cluster =[Glob_pos_acap_cluster, Glob_pos_acap_x_sort(jc+1)]
                    Glob_Strip_number_acap_cluster = [Glob_Strip_number_acap_cluster, nstrip_temp]
				    Glob_pair_flag_acap_cluster = [Glob_pair_flag_acap_cluster, 0]
                  endif
                endelse
		...
		; ------------------------------------
		; Y VIEW
		where_y = where(Glob_Si_id_acap_sameplane EQ 1)
		if (where_y(0) NE -1) then begin
			Glob_Strip_id_acap_y = Glob_Strip_id_acap_sameplane[where_y]
			Glob_tray_id_acap_y = Glob_tray_id_acap_sameplane[where_y]
			Glob_energy_dep_acap_y = Glob_energy_dep_acap_sameplane[where_y]
			Glob_pos_acap_y = Glob_pos_acap_sameplane[where_y]
			Glob_zpos_acap_y = Glob_zpos_acap_sameplane[where_y]
			
			;sorting the strips
			sort_ascending_strip = sort(Glob_Strip_id_acap_y)
			Glob_Strip_id_acap_y_sort = Glob_Strip_id_acap_y[sort_ascending_strip]
			Glob_tray_id_acap_y_sort = Glob_tray_id_acap_y[sort_ascending_strip]
			Glob_energy_dep_acap_y_sort = Glob_energy_dep_acap_y[sort_ascending_strip]
			Glob_pos_acap_y_sort = Glob_pos_acap_y[sort_ascending_strip]
			Glob_zpos_acap_y_sort = Glob_zpos_acap_y[sort_ascending_strip]
			
			e_cluster_temp = Glob_energy_dep_acap_y_sort(0)
            wx_cluster_temp = Glob_pos_acap_y_sort(0)*Glob_energy_dep_acap_y_sort(0)
            nstrip_temp = 1

            if (n_elements(Glob_Strip_id_acap_y_sort) EQ 1) then begin
            
            	Glob_event_id_cluster = [Glob_event_id_cluster, Glob_event_id_acap(j)]
            	Glob_Si_id_acap_cluster = [Glob_Si_id_acap_cluster, 1]
            	Glob_tray_id_acap_cluster = [Glob_tray_id_acap_cluster, Glob_tray_id_acap_y_sort(0)]
            	Glob_plane_id_acap_cluster = [Glob_plane_id_acap_cluster, Glob_plane_id_acap_temp_sortplane(pl)]
  				Glob_pos_acap_cluster =[Glob_pos_acap_cluster, total(wx_cluster_temp)/total(e_cluster_temp)]
  				Glob_zpos_acap_cluster = [Glob_zpos_acap_cluster, Glob_zpos_acap_y_sort(0)]
  				Glob_energy_dep_acap_cluster = [Glob_energy_dep_acap_cluster, total(e_cluster_temp)]
  				Glob_Strip_number_acap_cluster = [Glob_Strip_number_acap_cluster, nstrip_temp]
  				Glob_pair_flag_acap_cluster = [Glob_pair_flag_acap_cluster, 0]
              ...
              for jc=0l, n_elements(Glob_Strip_id_acap_y_sort) -2 do begin
                if (Glob_Strip_id_acap_y_sort(jc+1) EQ (Glob_Strip_id_acap_y_sort(jc)+2)) then begin
                  e_cluster_temp = [e_cluster_temp, Glob_energy_dep_acap_y_sort(jc+1)]
                  wx_cluster_temp = [wx_cluster_temp, Glob_pos_acap_y_sort(jc+1)*Glob_energy_dep_acap_y_sort(jc+1)]
                  nstrip_temp = nstrip_temp+1

                  if (jc EQ (n_elements(Glob_Strip_id_acap_y_sort)-2)) then begin
                    Glob_event_id_cluster = [Glob_event_id_cluster, Glob_event_id_acap(j)]
                    Glob_Si_id_acap_cluster = [Glob_Si_id_acap_cluster, 1]
                    Glob_tray_id_acap_cluster = [Glob_tray_id_acap_cluster, Glob_tray_id_acap_y_sort(jc)]
                    Glob_plane_id_acap_cluster = [Glob_plane_id_acap_cluster, Glob_plane_id_acap_temp_sortplane(pl)]
                    Glob_zpos_acap_cluster = [Glob_zpos_acap_cluster, Glob_zpos_acap_y_sort(jc)]
                    Glob_energy_dep_acap_cluster = [Glob_energy_dep_acap_cluster, total(e_cluster_temp)]
                    Glob_pos_acap_cluster =[Glob_pos_acap_cluster, total(wx_cluster_temp)/total(e_cluster_temp)]
                    Glob_Strip_number_acap_cluster = [Glob_Strip_number_acap_cluster, nstrip_temp]
                    Glob_pair_flag_acap_cluster = [Glob_pair_flag_acap_cluster, 0]
                  ...
                  e_cluster_temp = Glob_energy_dep_acap_y_sort(jc+1)
                  wx_cluster_temp = Glob_pos_acap_y_sort(jc+1)*Glob_energy_dep_acap_y_sort(jc+1)
                  nstrip_temp = 1

                  if (jc EQ (n_elements(Glob_Strip_id_acap_y_sort)-2)) then begin
                    Glob_event_id_cluster = [Glob_event_id_cluster, Glob_event_id_acap(j)]
                    Glob_Si_id_acap_cluster = [Glob_Si_id_acap_cluster, 1]
                    Glob_tray_id_acap_cluster = [Glob_tray_id_acap_cluster, Glob_tray_id_acap_y_sort(jc+1)]
                    Glob_plane_id_acap_cluster = [Glob_plane_id_acap_cluster, Glob_plane_id_acap_temp_sortplane(pl)]
                    Glob_zpos_acap_cluster = [Glob_zpos_acap_cluster, Glob_zpos_acap_y_sort(jc+1)]
                    Glob_energy_dep_acap_cluster = [Glob_energy_dep_acap_cluster, Glob_energy_dep_acap_y_sort(jc+1)]
				    Glob_pos_acap_cluster =[Glob_pos_acap_cluster, Glob_pos_acap_y_sort(jc+1)]
                    Glob_Strip_number_acap_cluster = [Glob_Strip_number_acap_cluster, nstrip_temp]
				    Glob_pair_flag_acap_cluster = [Glob_pair_flag_acap_cluster, 0]
                  endif
                endelse
		...
		if where_plane_eq(N_plane_eq-1) LT (n_elements(Glob_plane_id_acap_temp_sortplane)-1) then begin
	  		pl = where_plane_eq(N_plane_eq-1)+1
		endif else break
  	endwhile
  ...
  Glob_Si_id_acap_cluster = Glob_Si_id_acap_cluster[1:*]
  Glob_tray_id_acap_cluster = Glob_tray_id_acap_cluster[1:*]
  Glob_plane_id_acap_cluster = Glob_plane_id_acap_cluster[1:*]
  Glob_pos_acap_cluster = Glob_pos_acap_cluster[1:*]
  Glob_zpos_acap_cluster = Glob_zpos_acap_cluster[1:*]
  Glob_energy_dep_acap_cluster = Glob_energy_dep_acap_cluster[1:*]
  Glob_Strip_number_acap_cluster = Glob_Strip_number_acap_cluster[1:*]
  Glob_pair_flag_acap_cluster = Glob_pair_flag_acap_cluster[1:*]
  ...
  openw,lun,outdir+'/G4.AA.CLUSTER.AGILE'+agile_version+'.'+py_name+'.'+sim_name+'.'+stripname+'.'+sname+'.'+strmid(strtrim(string(N_in),1),0,10)+part_type+'.'+ene_type+'MeV.'+strmid(strtrim(string(theta_type),1),0,10)+'.'+strmid(strtrim(string(phi_type),1),0,10)+'.'+strtrim(string(ifile),1)+'.dat',/get_lun
  ; ASCII Columns:
  ; - c1 = event ID
  ; - c2 = theta input
  ; - c3 = phi input
  ...
  while (1) do begin
	...
	Glob_Si_id_acap_temp = Glob_Si_id_acap_cluster(where_event_eq)
	Glob_tray_id_acap_temp  = Glob_tray_id_acap_cluster(where_event_eq)
	Glob_plane_id_acap_temp  = Glob_plane_id_acap_cluster(where_event_eq)
	Glob_energy_dep_acap_temp = Glob_energy_dep_acap_cluster(where_event_eq)
	Glob_Strip_number_acap_cluster_temp = Glob_Strip_number_acap_cluster(where_event_eq)  
	Glob_pos_acap_temp = Glob_pos_acap_cluster(where_event_eq)
	Glob_zpos_acap_temp = Glob_zpos_acap_cluster(where_event_eq)
	...
	if (where_x(0) NE -1) then begin
	  for r=0l, n_elements(where_x)-1 do begin
		printf, lun, Glob_event_id_cluster(j), theta_type, phi_type, ene_type, Glob_plane_id_acap_temp(where_x(r)), Glob_zpos_acap_temp(where_x(r)), 0, Glob_pos_acap_temp(where_x(r)), Glob_energy_dep_acap_temp(where_x(r)), Glob_Strip_number_acap_cluster_temp(where_x(r)), 0, format='(I7,I7,I7,I7,I7,F20.7,I7,F20.7,F20.7,I7,I7)'
	  endfor
	endif
	...
	if (where_y(0) NE -1) then begin
	  for r=0l, n_elements(where_y)-1 do begin
		printf, lun, Glob_event_id_cluster(j), theta_type, phi_type, ene_type, Glob_plane_id_acap_temp(where_y(r)), Glob_zpos_acap_temp(where_y(r)), 1, Glob_pos_acap_temp(where_y(r)), Glob_energy_dep_acap_temp(where_y(r)), Glob_Strip_number_acap_cluster_temp(where_y(r)), 0, format='(I7,I7,I7,I7,I7,F20.7,I7,F20.7,F20.7,I7,I7)'
	  endfor
	endif
	; ------------------------------------
  ...
  Free_lun, lun
    ...
    openw,lun,outdir+'/G4_GAMS_YPLANE_AGILE'+agile_version+'_'+py_name+'_'+sim_name+'_'+stripname+'_'+sname+'_'+strmid(strtrim(string(N_in),1),0,10)+part_type+'_'+ene_type+'MeV_'+strmid(strtrim(string(theta_type),1),0,10)+'_'+strmid(strtrim(string(phi_type),1),0,10)+'.'+strtrim(string(ifile),1)+'.dat',/get_lun
    ; ASCII Columns:
    ; - c1 = event ID
    ; - c2 = plane ID
    ; - c3 = readout strip ID
    ...
    openw,lun,outdir+'/G4_GAMS_XPLANE_AGILE'+agile_version+'_'+py_name+'_'+sim_name+'_'+stripname+'_'+sname+'_'+strmid(strtrim(string(N_in),1),0,10)+part_type+'_'+ene_type+'MeV_'+strmid(strtrim(string(theta_type),1),0,10)+'_'+strmid(strtrim(string(phi_type),1),0,10)+'.'+strtrim(string(ifile),1)+'.dat',/get_lun
    ; ASCII Columns:
    ; - c1 = event ID
    ; - c2 = plane ID
    ; - c3 = readout strip ID
	...
	if (cal_flag) then begin
		...
		if (out_type EQ 0) then begin
		  ...
		  MWRFITS, calInput, outdir+'/G4.CAL.AGILE'+agile_version+'.'+py_name+'.'+sim_name+'.'+stripname+'.'+sname+'.'+strmid(strtrim(string(N_in),1),0,10)+part_type+'.'+ene_type+'MeV.'+strmid(strtrim(string(theta_type),1),0,10)+'.'+strmid(strtrim(string(phi_type),1),0,10)+'.'+strtrim(string(ifile),1)+'.fits', hdr_calInput, /create
		endif
	
		openw,lun,outdir+'/G4_GAMS_CAL_AGILE'+agile_version+'_'+py_name+'_'+sim_name+'_'+stripname+'_'+sname+'_'+strmid(strtrim(string(N_in),1),0,10)+part_type+'_'+ene_type+'MeV_'+strmid(strtrim(string(theta_type),1),0,10)+'_'+strmid(strtrim(string(phi_type),1),0,10)+'.'+strtrim(string(ifile),1)+'.dat',/get_lun
		; ASCII Columns:
		; - c1 = event ID
		; - c2 = bar plane 
		; - c3 = bar_id
	...
	if (ac_flag) then begin
		...
		if (out_type EQ 0) then begin
		  ...
		  MWRFITS, acInput, outdir+'/G4.AC.AGILE'+agile_version+'.'+py_name+'.'+sim_name+'.'+stripname+'.'+sname+'.'+strmid(strtrim(string(N_in),1),0,10)+part_type+'.'+ene_type+'MeV.'+strmid(strtrim(string(theta_type),1),0,10)+'.'+strmid(strtrim(string(phi_type),1),0,10)+'.'+strtrim(string(ifile),1)+'.fits', hdr_acInput, /create
		endif
	
		openw,lun,outdir+'/G4_GAMS_AC_AGILE'+agile_version+'_'+py_name+'_'+sim_name+'_'+stripname+'_'+sname+'_'+strmid(strtrim(string(N_in),1),0,10)+part_type+'_'+ene_type+'MeV_'+strmid(strtrim(string(theta_type),1),0,10)+'_'+strmid(strtrim(string(phi_type),1),0,10)+'.'+strtrim(string(ifile),1)+'.dat',/get_lun
		; ASCII Columns:
		; - c1 = event ID
		; - c2 = AC panel
		; - c3 = AC subpanel
